...
public class HCF extends JavaPlugin {
  ...
  private void saveData() {
    ...
    Command.broadcastCommandMessage(Bukkit.getConsoleSender(),
        "§aSauvegarde automatique des factions effectuée avec succès.");
  }
  ...
  public void runAutoSave() {
    ...
    }.runTaskTimerAsynchronously(this, 30, 18000);
  }
  ...
  /*
   * public PacketContainer buildTeamPacket(final String name, final String display, final String
   * prefix, final String suffix, final int flag, final String members) { final PacketContainer
   * packet = this.protocolManager.createPacket(PacketType.Play.Server.SCOREBOARD_TEAM);
   * packet.getIntegers().write(0, flag); packet.getStrings().write(0, name).write(1,
   * display).write(2, prefix).write(3, suffix);
   * packet.getSpecificModifier(Collection.class).write(0, Arrays.asList(members)); return packet; }
   */
