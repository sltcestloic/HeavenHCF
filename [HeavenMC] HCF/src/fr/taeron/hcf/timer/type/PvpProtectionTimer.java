...
public class PvpProtectionTimer extends PlayerTimer implements Listener
{
    ...
    public void onPlayerRespawn(final PlayerRespawnEvent event) {
        ...
        if (this.legible.add(player.getUniqueId())) {
            ...
            player.sendMessage(ChatColor.GREEN + "Quand tu quittera le spawn, tes 30 minutes de PvPTimer commencerons.");
            player.sendMessage(ChatColor.GREEN + "");
        }
    }
    ...
    public void onBucketEmpty(final PlayerBucketEmptyEvent event) {
        ...
        if (remaining > 0L) {
            ...
            player.sendMessage(ChatColor.RED + "Tu ne peux pas utiliser de sceau pendant que ton PvPTimer est actif.");
        }
    }
    ...
    public void onBlockIgnite(final BlockIgniteEvent event) {
        ...
        if (remaining > 0L) {
            event.setCancelled(true);
            player.sendMessage(ChatColor.RED + "Tu ne peux pas utiliser de briquet tant que ton PvPTimer est actif.");
        }
    }
    ...
    public void onPlayerClaimEnterMonitor(final PlayerClaimEnterEvent event) {
        ...
        if (fromFaction.isSafezone() && !toFaction.isSafezone()) {
            if (this.legible.remove(player.getUniqueId())) {
                ...
                player.sendMessage(ChatColor.GREEN + "Ton PvPTimer a commencé.");
                return;
            }
            if (this.getRemaining(player) > 0L) {
                this.setPaused(player, player.getUniqueId(), false);
                player.sendMessage(ChatColor.RED + "Ton PvPTimer n'est plus en pause.");
            }
        }
        else if (!fromFaction.isSafezone() && toFaction.isSafezone() && this.getRemaining(player) > 0L) {
            player.sendMessage(ChatColor.GREEN + "Ton PvPTimer est désormais en pause.");
    ...
    public void onPlayerClaimEnter(final PlayerClaimEnterEvent event) {
        ...
        if (toFaction instanceof ClaimableFaction && this.getRemaining(player) > 0L) {
            ...
            if (event.getEnterCause() == PlayerClaimEnterEvent.EnterCause.TELEPORT && toFaction instanceof PlayerFaction && (playerFaction = this.plugin.getFactionManager().getPlayerFaction(player)) != null && playerFaction.equals(toFaction)) {
                player.sendMessage(ChatColor.RED + "Tu es entré dans ton propre claim, ton PvPTimer a donc été retiré.");
                this.clearCooldown(player);
                return;
            }
            if (!toFaction.isSafezone() && !(toFaction instanceof RoadFaction)) {
                event.setCancelled(true);
                player.sendMessage(ChatColor.RED + "Tu ne peux pas entrer dans le territoire de " + toFaction.getDisplayName((CommandSender)player) + ChatColor.RED + " tant que ton PvPTimer est actif. Fait '" + ChatColor.GOLD + "/pvp enable" + ChatColor.RED + "' pour le désactiver.");
            }
        }
    }
    ...
    public void onEntityDamageByEntity(final EntityDamageByEntityEvent event) {
        ...
        if (entity instanceof Player) {
            ...
            if (this.getRemaining(player) > 0L) {
                event.setCancelled(true);
                attacker.sendMessage(ChatColor.RED + player.getName() + " est sous PvPTimer.");
                return;
            }
            if (this.getRemaining(attacker) > 0L) {
                event.setCancelled(true);
                attacker.sendMessage(ChatColor.RED + "Tu ne peux pas attaquer de joueurs tant que ton PvPTimer est actif."); 
            }
        }
    }
