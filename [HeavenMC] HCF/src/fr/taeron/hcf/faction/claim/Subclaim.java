...
public class Subclaim extends Claim implements Cloneable, ConfigurationSerializable {
    ...
    public Subclaim(final Map<String, Object> map) {
        super(map);
        (this.accessibleMembers = new HashSet<UUID>()).addAll((Collection<? extends UUID>)GenericUtils.createList(map.get("accessibleMembers"), String.class).stream().map(UUID::fromString).collect(Collectors.toList()));
    }
